...
export default function Home() {
  ...
  // State pour l'onglet de date actif
  ...
  // Fonction pour modifier le statut d'une tâche depuis l'accueil
  ...
  // --- LOGIQUE DES DATES POUR LE PLANNING ---
  ...
  // On filtre les tâches pour le planning
  ...
  return (
    <main className="min-h-screen bg-beige-300 p-8 text-slate-900">
      <div className="max-w-6xl mx-auto">
        ...
        {/* En-tête avec la JAUGE GLOBALE (Tes couleurs conservées) */}
        ...
        {/* NOUVEAU : LE WIDGET PLANNING (Adapté à ton thème rose) */}
        <div className="bg-white p-6 rounded-xl shadow-sm border border-pink-100 mb-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
            ...
            {/* Les onglets de dates */}
            <div className="flex bg-pink-50/50 p-1 rounded-lg overflow-x-auto border border-pink-100">
              <button onClick={() => setActiveDateTab('today')} className={`whitespace-nowrap px-4 py-2 text-sm font-semibold rounded-md transition-all ${activeDateTab === 'today' ? 'bg-white text-pink-600 shadow-sm' : 'text-pink-400 hover:text-pink-600'}`}>
                Aujourd'hui 🚨
              </button>
              <button onClick={() => setActiveDateTab('tomorrow')} className={`whitespace-nowrap px-4 py-2 text-sm font-semibold rounded-md transition-all ${activeDateTab === 'tomorrow' ? 'bg-white text-pink-600 shadow-sm' : 'text-pink-400 hover:text-pink-600'}`}>
                Demain
              </button>
              <button onClick={() => setActiveDateTab('dayAfter')} className={`whitespace-nowrap px-4 py-2 text-sm font-semibold rounded-md transition-all ${activeDateTab === 'dayAfter' ? 'bg-white text-pink-600 shadow-sm' : 'text-pink-400 hover:text-pink-600'}`}>
                Après-demain
              </button>
          ...
          {isFetchingData ? (
            <p className="text-pink-400">Chargement de ton planning...</p>
                  ...
                  <li key={task.id} className={`border p-4 rounded-lg flex flex-col justify-between gap-4 transition-all ${task.status === 'completed' ? 'bg-slate-50 border-slate-200 opacity-60' : isOverdue ? 'bg-red-50 border-red-200' : 'bg-white border-pink-100 shadow-sm'}`}>
                    ...
                    <div>
                      <div className="flex justify-between items-start mb-2">
                        <span className="text-xs font-bold px-2 py-1 rounded bg-pink-50 text-pink-600 uppercase">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {/* COLONNE GAUCHE : Les tâches en cours (Tes couleurs) */}
          <div className="lg:col-span-1">
            <div className="bg-white p-6 rounded-xl shadow-sm border border-amber-200 bg-amber-50/30 h-full">
              ...
              {isFetchingData ? (
                ...
                <p className="text-slate-500 text-sm italic">Aucune tâche en cours.</p>
              ) : (
          ...
          {/* COLONNE DROITE : Ajout et Liste des matières (Tes couleurs) */}
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-pink-100">
              ...
              <form onSubmit={handleAddSubject} className="flex gap-3">
                <input 
                  ...
                  disabled={isLoading}
                />
                <button type="submit" disabled={isLoading || !newSubject.trim()} className="bg-pink-600 text-white px-6 py-3 rounded-lg font-bold hover:bg-pink-700 disabled:bg-slate-300">
                  Ajouter
                </button>
              </form>
